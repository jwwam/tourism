...
import com.tourism.dao.FoodDao;
...
public class FoodServiceImpl implements FoodService {
    ...
    public String[] save(Food h) {
        return new String[0];
    }

    public void delete(Food h) {
        foodDao.delete(h);
    }
    ...
    public int upload(HttpServletRequest request, HttpServletResponse response) {
        ...
        try{
            ...
            if(StringUtils.isNotEmpty(h.getId())){
                int  result = foodDao.updateHotel(h.getAddress(), h.getDetail(), h.getImg(), h.getPrice(), h.getTitle(),  h.getId());
                if(result < 0) {
                    return -1;
                }else{
                    return 1;
